...
1. Convert the raw data to the processed data.
    ...
    3. To run it for all the datasets with one command: run `python local_convert_raw_to_processed.py`
2. Convert the processed data to the atomic data.
...
3. Train all algorithms on all datasets.
    ...
    3. To run it for all algorithms, datasets and folds for config 0: run `python local_run_fit.py`   
4. Create predictions with all algorithms on all datasets.
    ...
    2. To run locally:
       ...
       in `hpc_data_set_names.algorithm_names` for each `algorithm_config` retrieved by
       calling `recbole_algorithm_config.retrieve_configurations(algorithm_name)` for each `fold` in `[0,1,2,3,4]`.
    3. To run it for all algorithms, datasets and folds for config 0: run `python local_run_predict.py` 
5. Evaluate the predictions of all algorithms on all datasets.
    ...
    2. To run locally:
       ...
       in `hpc_data_set_names.algorithm_names` for each `algorithm_config` retrieved by
       calling `recbole_algorithm_config.retrieve_configurations(algorithm_name)` for each `fold` in `[0,1,2,3,4]`.
    3. To run it for all algorithms, datasets and folds for config 0: run `python local_run_evaluate.py`
6. Run `process_evaluation.py` to aggregate the results of the evaluations of all algorithms on all datasets and create
